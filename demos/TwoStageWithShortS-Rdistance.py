"""
coursework2_medium.py – Co-evolutionary Predator–Prey Simulation with Zones (Short Distance)
Short-distance layout variant: SafeZone and ResourceZone are nearly adjacent.

World layout (800 × 600):
    ResourceZone  – green circle at (480, 300)
                    Prey gain +1 food reward every timestep they spend inside.
    SafeZone      – red circle at (240, 300)
                    Predators are blocked from eating prey that are inside.
    The two zones are placed horizontally with only a ~10 px gap between
    their boundaries (centre-to-centre 240 px vs. radii sum 230 px).
...
IS_DEMO  = True
DEMO_NAME = "Coursework 2 - Two-Stage with Short S-R Distance"
...
"""
    ...
    SafeZone at (240, 300) and ResourceZone at (480, 300), placed horizontally
    on the same vertical axis. Centre-to-centre distance is 240 px while the
    sum of the radii is 230 px, leaving only a ~10 px corridor between the
    two zones – they are almost adjacent. This minimises travel cost between
    safety and reward, removing most of the safety/reward trade-off.
"""
SAFE_ZONE_CENTER  = np.array([240.0, 300.0], np.float32)
SAFE_ZONE_RADIUS  = 80.0

RESOURCE_ZONE_CENTER = np.array([480.0, 300.0], np.float32)
...
class CoevSimulationTwoStage(Simulation):
    ...
    Short-distance layout variant: SafeZone and ResourceZone almost adjacent.
    """
    ...
    def __init__(self):
        ...
        self._zones = [
            SafeZone(SAFE_ZONE_CENTER),   # at (240, 300)
            ResourceZone(),                # at (480, 300)
        ]
    ...
    def _save_results(self) -> None:
        ...
        filename = os.path.join(
            results_dir, f"TwoStageEvolutionWithShortSR-Distance_{self.generations}gens.csv"
        )
    ...
    def _plot_results(self, results_dir: str) -> None:
        ...
        plot_path = os.path.join(
            results_dir, f"TwoStageEvolutionWithShortSR-Distance_{self.generations}gens.png"
        )
